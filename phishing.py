...
c = conn.cursor()
c.execute("SELECT url from url where isparsed = -1 and status = 'up'")
rows = c.fetchall()
for row in rows:
  url = row[0]
  print(url) 
#url= "http://www.futuredent.co.uk/www.made-in-china.com/Made-in-China.htm"
#url = "https://peringatanakun291.wixsite.com/mysite"

#url="http://protienfish.tk/adobepdf/nsw/data/"
  try:
    ...
    print([cdash, cdot, chtps, clen , creq, canc, cage, cpop])
    c.execute("update url set checkdash = ?, checkdot = ?, checkifhttps = ?, checklen = ?, checkreq = ?, checkanch = ?, checkage = ?, checkpop = ?, isparsed = 1, status = 'up' where url = ?", (cdash, cdot, chtps, clen , creq, canc, cage, cpop, url))
    conn.commit()
  except Exception as e:
   c.execute("update url set status = ? where url = ?", ("down", url))
   conn.commit()
   print("Site down")
